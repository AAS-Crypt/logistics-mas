...
public class SensitivityAnalyzer {
    ...
    private static void runAnalysis(String parameter, String range, int scenarios) {
        ...
        for (double value = start; value <= end + 0.0001; value += step) {
            ...
            Logger.info("SensitivityAnalyzer", "  Results: Cost=$" + String.format("%.2f", avgCost) + 
                       ", Time=" + String.format("%.1f", avgTime) + "h, Success=" + 
                       String.format("%.1f", avgSuccessRate * 100) + "%, Escalations=" + 
                       String.format("%.1f", avgEscalations));
        ...
        Logger.info("SensitivityAnalyzer", "Parameter: " + parameter);
        Logger.info("SensitivityAnalyzer", "Range: " + range);
        Logger.info("SensitivityAnalyzer", "Optimal value based on combined metrics: " + 
                   String.format("%.2f", (start + end) / 2));
