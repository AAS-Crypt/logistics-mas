...
function run_basic_tests() {
    ...
    $MVN_CMD test -Dtest=MCAATest 2>&1 | tee "$TEST_DIR/test_t01_mcaa.log"
    
    echo "2. T-02: Contract Net Protocol Test"
    $MVN_CMD test -Dtest=ContractNetTest 2>&1 | tee "$TEST_DIR/test_t02_contractnet.log"
    ...
    $MVN_CMD test -Dtest=CAGATest 2>&1 | tee "$TEST_DIR/test_t04_caga.log"
    
    echo "5. T-05: AERA Impact Calculation Test"
    $MVN_CMD test -Dtest=AERATest 2>&1 | tee "$TEST_DIR/test_t05_aera.log"
    
    echo "6. T-06: PCRA Cluster Optimization Test"
    run_java com.logistics.simulator.PCRASimulator 2>&1 | tee "$TEST_DIR/test_t06_pcra.log"
    
    echo "7. T-07: SPTA Policy Update Test"
    $MVN_CMD test -Dtest=SPTATest 2>&1 | tee "$TEST_DIR/test_t07_spta.log"
}
...
function run_extended_scenarios() {
    ...
    run_java com.logistics.simulator.ScenarioRunner --scenario high_priority --orders 10 --urgency 0.8 2>&1 | tee "$TEST_DIR/scenario_high_priority.log"
    
    echo "2. Low Budget Order Test"
    run_java com.logistics.simulator.ScenarioRunner --scenario low_budget --budget_multiplier 0.5 2>&1 | tee "$TEST_DIR/scenario_low_budget.log"
    
    echo "3. Multiple Resources Test"
    run_java com.logistics.simulator.ScenarioRunner --scenario multiple_resources --resources 10 --orders 50 2>&1 | tee "$TEST_DIR/scenario_multiple_resources.log"
    
    echo "4. Edge Case - Extreme Urgency"
    run_java com.logistics.simulator.ScenarioRunner --scenario extreme_urgency --deadline_multiplier 0.1 2>&1 | tee "$TEST_DIR/scenario_extreme_urgency.log"
}
...
function run_uncertainty_analysis() {
    ...
    run_java com.logistics.simulator.MonteCarloSimulator --iterations 100 --uncertainty_level medium 2>&1 | tee "$TEST_DIR/monte_carlo_medium.log"
    
    echo "2. Sensitivity Analysis for MCAA weights"
    run_java com.logistics.simulator.SensitivityAnalyzer --parameter "mcaa_time_weight" --range "0.1:0.9:0.1" --scenarios 5 2>&1 | tee "$TEST_DIR/sensitivity_time_weight.log"
    
    echo "3. Algorithm Comparison Under Uncertainty"
    run_java com.logistics.benchmark.AlgorithmComparator --algorithms "MCAA,GA,PSO,Random" --scenarios "uncertainty" --iterations 20 2>&1 | tee "$TEST_DIR/algorithm_comparison.log"
}
...
function generate_report() {
    ...
    if [ -f "analyze_results.py" ]; then
        python analyze_results.py --input_dir "$TEST_DIR" --output "$TEST_DIR/research_report.md"
